...
CREATE TABLE es_product_category (
		...
		`show_image` VARCHAR ( 2600 ) COMMENT '显示图片或图标',
...
CREATE TABLE es_shop (
		...
		`show_image` VARCHAR ( 2600 ) COMMENT '展示图片',
...
CREATE TABLE es_sku_product (
		...
		`name` VARCHAR(64) COMMENT 'sku商品名称',
		`show_image` VARCHAR(2600) COMMENT '展示图片',
...
CREATE TABLE es_coupon (
		...
		`use_range` INT(1) COMMENT '适用范围, 0 -> 指定商品, 1 -> 无门槛, 2 -> 店铺商品, 3 -> 分类商品',
		...
		`start_point` DECIMAL(10,2) COMMENT '优惠券使用起点',
...
-- 秒杀商品表
DROP TABLE IF EXISTS es_flash_sale_product;
